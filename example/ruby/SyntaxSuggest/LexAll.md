...
This class guarantees the whole document is lex-ed by iteratively lexing the
document where ripper stopped.

Prism likely doesn't have the same problem. Once ripper support is removed we
can likely reduce the complexity here if not remove the whole concept.
...
# Class Methods
## lex(source , line_number ) [](#method-c-lex)
...
## initialize(source:, source_lines:nil) [](#method-i-initialize)
