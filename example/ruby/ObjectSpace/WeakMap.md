...
An ObjectSpace::WeakMap is a key-value map that holds weak references to its
keys and values, so they can be garbage-collected when there are no more
references left.
...
See also ObjectSpace::WeakKeyMap map class, which compares keys by value, and
holds weak references only to the keys.
...
Iterates over keys. Note that unlike other collections, `each_key` without
...
Iterates over values. Note that unlike other collections, `each_value` without
block isn't supported.
