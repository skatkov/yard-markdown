...
## add(param) [](#method-i-add)
Add the SvcParam `param` to this list, overwriting the existing one with the
same key.
...
`params` has to be an enumerable of +SvcParam+s. If its content has
+SvcParam+s with the duplicate key, the one appears last takes precedence.

**@return** [SvcParams] a new instance of SvcParams
