...
## each_source(&b) [](#method-i-each_source)
...
Returns true if this source list includes `other` which may be a Gem::Source
or a source URI.
...
## replace(other) [](#method-i-replace)
Replaces this SourceList with the sources in `other`  See #<< for acceptable
items in `other`.
